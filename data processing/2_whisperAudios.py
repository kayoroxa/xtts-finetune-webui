...
# --- Configurações ---
FOLDER_NAME           = "wavs"
PASTA_CORTES          = BASE_DIR / "audios" / FOLDER_NAME
...
arquivos_wav = sorted(
    [f for f in PASTA_CORTES.glob("*") if f.suffix.lower() in [".wav", ".mp3"] and f.is_file()]
)
        ...
        try:
            ...
            if segments:
                ...
                if CHECK_CONFIDENCE and media_logprob < CONFIDENCE_THRESHOLD:
                    print(f"⚠️ Ignorando {arquivo.name} por baixa confiança {media_logprob} is (limiar={CONFIDENCE_THRESHOLD})")
            ...
            if texto_completo:
                writer.writerow([f"{FOLDER_NAME}/{arquivo.name}", texto_completo, SPEAKER_NAME])
